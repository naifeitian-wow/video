...
def get_movie(page):
    url='http://5566.feifei6688.cn/index.php/vod/type/id/2/page/{}/'.format(page)
    ...
    for list in lists:
        ...
        cursor = db.cursor()
        try:
            cursor.execute('SELECT * FROM tv where title="{}"'.format(title))
            one = cursor.fetchone()
            if one:
                cursor.execute(
                    'update tv set title="{}",pic="{}",type="{}",country="{}",year="{}",director="{}",zhuyan="{}",story="{}",url="{}",time="{}",url_lists="{}" where title="{}"'.format(
                        title, pic, type, country, year, director, zhuyan, story, url, time,url_lists, title))
                db.commit()
                print('update tv ok')
            else:
                cursor.execute(
                    'insert into tv(title,pic,type,country,year,director,zhuyan,story,url,time,category_id,url_lists)VALUES ("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}",1,"{}")'.format(title,pic,type,country,year,director,zhuyan,story,url,time,url_lists))
                db.commit()
